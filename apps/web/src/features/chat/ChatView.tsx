...
export function ChatView({ catalog, balance, api }: Props): React.ReactElement {
  ...
  React.useEffect(() => {
    void api.request<{ balance: number }>({ routeKey: "wallet-get" })
      .then((wallet) => setAvailableBalance(wallet.balance))
      .catch(() => undefined);
  }, [api]);
