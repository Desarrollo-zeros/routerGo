...
| P0-003 | Economy is debit/refund, not QUOTE → RESERVE → EXECUTE → SETTLE → RELEASE/REFUND. | **Partially resolved 2026-08-15 by T020-T022**: additive migration `004_economy_accounting.sql` provides durable credit reservations, finite unit-aware budgets, provider-cost records, revenue records, fixed-precision units, idempotency keys, and deletion constraints. T021 now enforces reservation lifecycle invariants and T022 provides deny-by-default budget/circuit policy. Unit-of-work orchestration, concurrency/idempotency integration, provider execution, and reconciliation remain in T023-T026. | Durable accounting and framework-free domain policy exist, but public API/ads rollout remains blocked until application flows enforce non-negative/concurrency-safe reserve accounting and reconciliation. |
...
Do not begin Phases 3–10. T003, T004, T005, T006, T007, T008, T010, T011, T012, T013, T014, T015, T020, T021, and T022 are now closed locally. T023 is the next dependency-unlocked slice; T024-T026 depend on the application flow and persistence contracts. Authentication transport and HTTP authorization enforcement remain later integration work.
