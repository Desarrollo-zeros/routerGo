...
- T101 foundation connected: `/admin/economy` is an explicit `economy.read` API-key capability route, and Studio fetches its typed unit-economics response when an authenticated session injects an access token. A browser session/token transport and operator RBAC remain T042-T044 concerns.
