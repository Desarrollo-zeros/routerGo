...
| P0-003 | Economy is debit/refund, not QUOTE → RESERVE → EXECUTE → SETTLE → RELEASE/REFUND. | **Resolved through T025 on 2026-08-15**: T020-T024 provide durable reservations, command idempotency, unit-aware budgets, provider-cost records, fixed-precision units, and locking. T025 gates the selected platform budget, claims the run in PostgreSQL, reserves before provider execution, settles/releases after the provider result, persists request/usage/cost metadata, and marks compensation failures or ambiguous delivery for reconciliation. Durable platform-cost commitment and reconciliation workers remain T026. | User-credit execution is fail-closed and auditable at the run boundary; public rollout remains blocked until provider adapters, platform commitment/reconciliation, and HTTP authorization are complete. |
...
Do not begin Phases 3–10. T003, T004, T005, T006, T007, T008, T010, T011, T012, T013, T014, T015, T020, T021, T022, T023, T024, and T025 are closed locally. T026 depends on the execution lifecycle contracts. Authentication transport and HTTP authorization enforcement remain later integration work.
