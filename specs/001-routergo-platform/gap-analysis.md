...
- T082 is implemented: Redis stores validated ephemeral match state with an explicit TTL, while migration `014_battle_results.sql` and `PostgresBattleResultRepository` durably record one server-owned result per match idempotently. Reconnect handling and reward integration remain T083.
