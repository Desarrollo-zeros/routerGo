...
| P0-003 | Economy is debit/refund, not QUOTE → RESERVE → EXECUTE → SETTLE → RELEASE/REFUND. | **Partially resolved 2026-08-15 by T020-T023**: migrations `004_economy_accounting.sql` and `005_credit_reservation_operations.sql` provide durable reservations, command idempotency, finite unit-aware budgets, provider-cost records, revenue records, fixed-precision units, and deletion constraints. T021 enforces reservation invariants, T022 provides deny-by-default budget/circuit policy, and T023 atomically orchestrates user-credit reserve/settle/release with PostgreSQL locking. Quote/run integration, broader concurrency stress, provider execution, platform-budget commitment, and reconciliation remain in T024-T026. | Durable user-credit accounting and framework-free policy exist, but public API/ads rollout remains blocked until quote/run integration, broader concurrency coverage, and reconciliation are complete. |
...
Do not begin Phases 3–10. T003, T004, T005, T006, T007, T008, T010, T011, T012, T013, T014, T015, T020, T021, T022, and T023 are now closed locally. T024 is the next economy-hardening slice; T025-T026 depend on the application flow and persistence contracts. Authentication transport and HTTP authorization enforcement remain later integration work.
