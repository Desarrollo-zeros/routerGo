...
| P0-003 | Economy is debit/refund, not QUOTE → RESERVE → EXECUTE → SETTLE → RELEASE/REFUND. | **Partially resolved 2026-08-15 by T020-T024**: migrations `004_economy_accounting.sql` and `005_credit_reservation_operations.sql` provide durable reservations, command idempotency, finite unit-aware budgets, provider-cost records, revenue records, fixed-precision units, and deletion constraints. T021 enforces reservation invariants, T022 provides deny-by-default budget/circuit policy, T023 atomically orchestrates user-credit reserve/settle/release with PostgreSQL locking, and T024 verifies real concurrent contention, duplicate callbacks, payload conflicts, rollback retries, and exact reconciliation. Quote/run integration, provider execution, platform-budget commitment, and reconciliation remain in T025-T026. | Durable user-credit accounting and framework-free policy exist, but public API/ads rollout remains blocked until quote/run integration, provider execution, platform-budget commitment, and reconciliation are complete. |
| P0-004 | The runtime API is not the specified RouterGo API and has no effective HTTP authorization enforcement. | Seeded routes are legacy paths such as `/catalog`, `/quotes`, and `/runs`; `/v1/models` is absent. `DynamicRouteRegistry` stores `auth_policy_key` but never invokes the converged identity/RBAC application flow. T011/T012 provide the framework-free contracts and policy, but authentication transport, API keys, route middleware, rate limits, and budgets remain absent. | Public API and admin routes can be exposed without the required credential/security boundary. |
...
Do not begin Phases 3–10. T003, T004, T005, T006, T007, T008, T010, T011, T012, T013, T014, T015, T020, T021, T022, T023, and T024 are now closed locally. T025 is the next economy slice; T026 depends on the application flow and persistence contracts. Authentication transport and HTTP authorization enforcement remain later integration work.
