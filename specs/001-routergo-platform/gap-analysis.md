...
| P0-003 | Economy is debit/refund, not QUOTE → RESERVE → EXECUTE → SETTLE → RELEASE/REFUND. | **Partially resolved 2026-08-15 by T020**: additive migration `004_economy_accounting.sql` now provides durable credit reservations, finite unit-aware budgets, provider-cost records, revenue records, fixed-precision units, idempotency keys, and deletion constraints. Reservation/settlement behavior, concurrency orchestration, and economy circuit-breaker policy remain unimplemented in T021-T026. | Durable accounting storage exists, but public API/ads rollout remains blocked until application flows enforce non-negative/concurrency-safe reserve accounting and reconciliation. |
...
Do not begin Phases 3–10. T003, T004, T005, T006, T007, T008, T010, T011, T012, T013, T014, T015, and T020 are now closed locally. T021 and T022 are the next dependency-unlocked slices and can proceed in parallel; T023 depends on both. Authentication transport and HTTP authorization enforcement remain later integration work.
